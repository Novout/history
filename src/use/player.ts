...
export const usePlayer = () => {
  ...
  const isAdjacentTerritory = (player: HistoryPlayer | null, target: HistoryTerrain): boolean => {
    if(player === null) return false

    const isAdjacent = getTerritories(player).find((t) => utils.isAdjacentHex(t, target))
  ...
  const getAllAdjacentTerritories = (player: HistoryPlayer | null) => {
    ...
    return APP.terrain.filter(f => isAdjacentTerritory(player, f)).filter(f => f.owner !== player.name)
  }
  ...
  const getFood = (player: HistoryPlayer | null) => {
    ...
    return getTerritories(player).reduce((sum, acc) => {
      return sum += (acc.city ? (acc.resources.food + acc.city.resources.food) : 0) + (acc.structure ? getStructureValue(acc.structure).food : 0)
    }, 0)
  }
  ...
  const getProduction = (player: HistoryPlayer | null) => {
    ...
    return getTerritories(player).reduce((sum, acc) => {
      return sum += (acc.city ? (acc.resources.production + acc.city.resources.production) : 0) + (acc.structure ? getStructureValue(acc.structure).production : 0)
    }, 0)
  }
  ...
  const getScience = (player: HistoryPlayer | null) => {
    ...
    return getTerritories(player).reduce((sum, acc) => {
      return sum += (acc.city ? (acc.resources.science + acc.city.resources.science) : 0) + (acc.structure ? getStructureValue(acc.structure).science : 0)
    }, 0)
  }
    ...
    switch(type) {
      ...
      case 'academic_center':
        return {
          ...
          production: COST_DEFINE.CONSTRUCTION_ACADEMIC_CENTER_UPGRADE_PRODUCTION[0]
        }
    }
  }
  ...
  const getStructureValue = (type: HistoryTerrainStructure) => {
    ...
    return {
      ...
      production: define?.resources?.production ? define?.resources?.production[0] || 0 : 0,
      influence: define?.resources?.influence ? define?.resources?.influence[0] || 0 : 0,
      science: define?.resources?.influence ? define?.resources?.science[0] || 0 : 0,
      multiplier: define?.multiplier || undefined
    }
  }

  return { isAdjacentTerritory, getAllAdjacentTerritories, getTerritories, getInfluence, getFood, getProduction, getScience, getMilitaryCapacity, getStructureCost, getStructureValue }
}