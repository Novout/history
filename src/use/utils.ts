...
export const useUtils = () => {
  ...
  const getRandomTerrainID = (forceOwner: boolean = false, ignoreNotColonizableTerrain: boolean = true): number => {
  ...
  const getRandomColor = (): number => {
    const hex = Math.floor(Math.random()*16777215).toString(16).replace('#', '');
  ...
  const isAdjacentHex = (hex1: HistoryTerrain, hex2: HistoryTerrain): boolean => {
