<template>
  <div class="p-10 h-100 select-none overflow-y-auto w-35 gap-2 fixed top-1/2 transform -translate-y-1/2 text-white right-0 z-max bg-blur flex flex-col rounded-t-md">
    <H2>Cidades</H2>
    <div v-for="(terrain, index) in player.getCityTerritories(APP.player)" :key="index">
      <div @click="onTerrainInfo(terrain.id)" class="flex w-full gap-2 items-center cursor-pointer">
        <div>
          <IconCity class="w-5 h-5" />
        </div>
        <H3 class="truncate">{{ terrain.city?.name }} {{ terrain.city?.structure.townHall }}</H3>
      </div>
    </div>
